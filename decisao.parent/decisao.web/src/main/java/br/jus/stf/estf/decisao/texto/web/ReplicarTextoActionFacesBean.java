...
public class ReplicarTextoActionFacesBean extends
	...
	private void execute() {
		...
		try {
			...
			try {
				objetoIncidenteService.registrarLogSistema(getTextoSelecionado().getIdObjetoIncidente(), "CONSULTA_TEXTO", "Criar Replica de Texto: "+getTextoSelecionado().getId()+" - "+getTextoSelecionado().getTipoTexto().getDescricao(),texto.getId(),"STF.TEXTOS");
		...
		if (tipoTexto.equals(TipoTexto.ACORDAO) || tipoTexto.equals(TipoTexto.RELATORIO) || tipoTexto.equals(TipoTexto.EMENTA)) {
			for (TextoDto textoObjetoIncidente : textoService.recuperarTextos(objetoIncidenteService.recuperarObjetoIncidentePorId(objetoIncidente.getId()), false)) {
				if (textoObjetoIncidente.getTipoTexto().equals(tipoTexto)) {
					throw new ManterListaDeTextosException("O Texto com o tipo [" + tipoTexto.getDescricao() + "] já existe no processo " + objetoIncidente.getIdentificacao() + ".");
				}
			}
			...
			for (TextoDto textoObjetoIncidente : textoService.recuperarTextos(objetoIncidenteService.recuperarObjetoIncidentePorId(objetoIncidente.getId()), false)) {
				...
				if (textoObjetoIncidente.getTipoTexto().equals(tipoTexto) && ministro.getId().equals(textoObjetoIncidente.getIdMinistro())) {
						String mensagem =  String.format("O Texto com o tipo [%s] já existe no processo %s para %s %s.",tipoTexto.getDescricao(), objetoIncidente.getIdentificacao(), getArtigoDoMinistro(ministro),ministro.getNome() );
