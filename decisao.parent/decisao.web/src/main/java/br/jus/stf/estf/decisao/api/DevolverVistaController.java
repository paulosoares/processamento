...
@Controller
public class DevolverVistaController extends HttpServlet {
	...
	public DevolverVistaController() {
		super();
	}

	private List<ReferendarDecisaoResultadoDto> devolverVista(List<ReferendarDecisaoDto> lista, Principal principal) {
		try {
			devolverVistaService = (DevolverVistaService) getService("devolverVistaService");
			List<ReferendarDecisaoResultadoDto> result = devolverVistaService.devolverVista(lista, principal);
			return result;
		} catch (Exception e) {
			ArrayList<ReferendarDecisaoResultadoDto> erro = new ArrayList<ReferendarDecisaoResultadoDto>();
			erro.add(new ReferendarDecisaoResultadoDto(null, TipoMensagem.FALHA, e.getMessage(), null, null));
			return erro;
		}
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//ignora o CPC por padrão quando o parâmetro não for enviado
		for (ReferendarDecisaoDto item : lista)
			if (item.getIgnorarCpc() == null)
				item.setIgnorarCpc(true);

		List<ReferendarDecisaoResultadoDto> resultado = devolverVista(lista, getPrincipal());
