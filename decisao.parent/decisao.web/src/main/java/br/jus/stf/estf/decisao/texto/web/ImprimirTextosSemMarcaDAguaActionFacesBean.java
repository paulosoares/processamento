 ...
 * @see 20.07.2010
 */
@Action(id="imprimirTextosSemMarcaDAguaActionFacesBean", name="Imprimir Textos Sem Marca D'Água", report = true)
@Restrict({ActionIdentification.IMPRIMIR_SEM_MARCA_D_AGUA})
...
public class ImprimirTextosSemMarcaDAguaActionFacesBean extends ReportAction<TextoDto> {
		...
		try {
			if (resources.size() == 1 && FaseTexto.fasesComTextoAssinado.contains(resources.iterator().next().getFase())) {
				...
				if (documentoTexto == null) {
					...
					DadosMontagemTexto<Long> dadosMontagem = dadosMontagemTextoBuilder.montaDadosMontagemTexto(texto, false);
					...
					try {
						objetoIncidenteService.registrarLogSistema(texto.getObjetoIncidente(), "CONSULTA_TEXTO", "Imprimir Texto Sem marca D'agua: "+texto.getId()+" - "+texto.getTipoTexto().getDescricao(),texto.getId(),"STF.TEXTOS");
