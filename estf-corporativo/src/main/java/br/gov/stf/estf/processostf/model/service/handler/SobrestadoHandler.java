...
public class SobrestadoHandler extends AndamentoProcessoHandler {
	...
	public String getSituacao(AndamentoProcesso andamentoProcesso) throws ServiceException {
		...
		exemplo.setTipoDependenciaProcesso(TipoProcessoDependenciaEnum.SOBRESTADO.getCodigo());
		
		List<ProcessoDependencia> dependencias = processoDependenciaService.pesquisarPorExemplo(exemplo);
		
		String descricao = "Aguardando Julgamento: ";
	...
	public void preRegistroAndamento(AndamentoProcesso andamentoProcesso, Processo processoAndamento, List<Processo> processosPrincipais, Peticao peticao, Setor setor,
		...
		for (Iterator iterator = processosPrincipais.iterator(); iterator.hasNext();) {
			
			Processo processoPrincipal = (Processo) iterator.next();
			if (processoAndamento.getIdentificacao().equals(processoPrincipal.getIdentificacao())) {
				throw new ServiceException("Um processo não pode ser sobrestado a ele mesmo.");
			}
		}
	}
	@Override
	public void posRegistroAndamento(AndamentoProcesso andamentoProcesso, Processo processoAndamento, List<Processo> processosPrincipais, Peticao peticao, Setor setor, String codigoUsuario, Origem origem, Comunicacao comunicacao) throws ServiceException {
		...
		for (Iterator iterator = processosPrincipais.iterator(); iterator.hasNext();) {
			...
			Processo processoPrincipal = (Processo) iterator.next();
			processoDependenciaService.criarDependencia(andamentoProcesso, processoPrincipal, ProcessoDependencia.TipoProcessoDependenciaEnum.SOBRESTADO.getCodigo());
		}
	}
}