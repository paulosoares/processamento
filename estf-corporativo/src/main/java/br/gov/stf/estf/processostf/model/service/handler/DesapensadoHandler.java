...
public class DesapensadoHandler extends AndamentoProcessoHandler {
	...
	public String getSituacao(AndamentoProcesso andamentoProcesso) throws ServiceException {
		...
		exemplo.setTipoDependenciaProcesso(TipoProcessoDependenciaEnum.DESAPENSADO_DO_PROCESSO_NO.getCodigo());
	...
	public void preRegistroAndamento(AndamentoProcesso andamentoProcesso, Processo processoAndamento, List<Processo> processosPrincipais, Peticao peticao, Setor setor, String codigoUsuario, Origem origem) throws ServiceException {
		...
		// O processo deve estar previamente apensado.
		if (!processoDependenciaService.isProcessoApensado(processoAndamento, processosPrincipais.get(0))) {
			throw new DesapensadoException("Não é possível desapensar o processo '" + processoAndamento.getSiglaClasseProcessual() + " " + processoAndamento.getNumeroProcessual() + "' porque ele não está apensado ao processo '"+ 
					processosPrincipais.get(0).getSiglaClasseProcessual() + " " + processosPrincipais.get(0).getNumeroProcessual() + "'.");
		}
	}
	...
	public void posRegistroAndamento(AndamentoProcesso andamentoProcesso, Processo processoAndamento, List<Processo> processosPrincipais, Peticao peticao, Setor setor, String codigoUsuario, Origem origem, Comunicacao comunicacao) throws ServiceException {
		...
		processoDependenciaService.criarDependencia(andamentoProcesso, processosPrincipais.get(0), ProcessoDependencia.TipoProcessoDependenciaEnum.DESAPENSADO_DO_PROCESSO_NO.getCodigo());
		processoDependenciaService.finalizarApenso(processoAndamento, processosPrincipais.get(0));
	}
}